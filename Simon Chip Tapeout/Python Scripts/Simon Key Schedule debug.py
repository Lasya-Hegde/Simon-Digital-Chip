...
class SimonCipher(object):
    ...
    def __init__(self, key, key_size=128, block_size=128, mode='ECB', init=0, counter=0):
        ...
        print(f"Initial key subwords (k_init): {[hex(k) for k in k_init]}")

        k_reg = deque(k_init)  # Use queue to manage key subwords
        print(f"Initial key register (k_reg): {[hex(k) for k in list(k_reg)]}")

        round_constant = self.mod_mask ^ 3  # Round Constant is 0xFFFF..FC
        print(f"Round constant: {hex(round_constant)}")
        ...
        for x in range(self.rounds):
            ...
            print(f"rs_3 (rotated left by 3): {hex(rs_3)}")

            if m == 4:
                rs_3 ^= k_reg[2]
                print(f"rs_3 after XOR with k_reg[2]: {hex(rs_3)}")

            rs_1 = ((rs_3 << (self.word_size - 1)) + (rs_3 >> 1)) & self.mod_mask
            print(f"rs_1 (rotated left by 1): {hex(rs_1)}")

            c_z = ((self.zseq >> (x % 62)) & 1) ^ round_constant
            print(f"c_z (zseq bit XOR round_constant): {hex(c_z)}")

            new_k = c_z ^ rs_1 ^ rs_3 ^ k_reg[m - 1]
            print(f"new_k: {hex(new_k)}")
            print(f"c_z: {hex(c_z)}")
            print(f"rs_1: {hex(rs_1)}")
            print(f"rs_3: {hex(rs_3)}")
            print(f"k_reg[{m - 1}]: {hex(k_reg[m - 1])}")


            popped = k_reg.pop()
            print(f"popped value from k_reg: {hex(popped)}")
            self.key_schedule.append(popped)

            k_reg.appendleft(new_k)
            print(f"Updated k_reg: {[hex(k) for k in k_reg]}")
