...
class SimonCipher(object):
    ...
    def encrypt_function(self, upper_word, lower_word):
        ...
        print(f"Initial values: x = {hex(x)}, y = {hex(y)}")
        print(f"Number of rounds = {len(self.key_schedule)}")

        # Run Encryption Steps For Appropriate Number of Rounds
        for i, k in enumerate(self.key_schedule):
            print(f"\nRound {i}:")
            print(f"Round key (k) = {hex(k)}")
            ...
            print(f"ls_1_x = {hex(ls_1_x)}")
            print(f"ls_8_x = {hex(ls_8_x)}")
            print(f"ls_2_x = {hex(ls_2_x)}")

            # XOR Chain
            xor_1 = (ls_1_x & ls_8_x) ^ y
            xor_2 = xor_1 ^ ls_2_x

            print(f"xor_1 = {hex(xor_1)} (ls_1_x & ls_8_x) ^ y")
            print(f"xor_2 = {hex(xor_2)} xor_1 ^ ls_2_x")

            y = x
            x = k ^ xor_2

            print(f"New x = {hex(x)} (k ^ xor_2)")
            print(f"New y = {hex(y)} (previous x)")

        print(f"\nFinal ciphertext: x = {hex(x)}, y = {hex(y)}")
        return x, y
